...
def run_formatter(command: list[str]):
    ...
    try:
        ...
        print(f"✅ {cmd_str} completed successfully!")
        if result.stdout:
            print(f"Output: {result.stdout}")
        return True
    except subprocess.CalledProcessError as e:
        print(f"❌ {cmd_str} failed: {e}")
        if e.stderr:
            print(f"Error output: {e.stderr}")
        return False
    except FileNotFoundError:
        print(
            f"❌ {command[0]} not found. Make sure it is installed and in PATH."
        )
        return False
    except UnicodeDecodeError as e:
        print(f"❌ {cmd_str} encoding error: {e}")
        # Tenta executar sem capturar a saída
        try:
            subprocess.run(command, check=True)
            print(
                f"✅ {cmd_str} completed successfully (without output capture)!"
            )
            return True
...
if __name__ == "__main__":
    ...
    # run ruff format and ruff check on __init__.py
    run_formatter(["ruff", "format", init_file])
    run_formatter(["ruff", "check", "--fix", init_file])
