...
def definition_hash(definition: Mapping[str, Any]) -> str:
    ...
    for part in sorted(definition.get("parts", []), key=lambda p: p["path"]):
        path = part["path"]
        content = _canonical(path, base64.b64decode(part["payload"]))
...
def _canonical(path: str, content: bytes) -> bytes:
    """Put one part in canonical form; bytes that are not UTF-8 stay."""
    try:
