...
def export_all_reports(
    ...
    for item in items:
        ...
        if definition is None:
            logger.error(
                ...
                f"{item['displayName']}.Report; skipping it."
            )
    ...
    if failed:
        logger.warning(
            f"{len(failed)} report(s) could not be exported: "
            f"{', '.join(failed)}."
        )
    else:
...
def deploy_all_reports(
    ...
    Shortcut for ``deploy_all_items(..., item_types=["Report"])``.
    ...
    Returns:
        DeploymentReport: The outcome of each report.
    """
    return deploy_all_items(workspace, path, start_path, item_types=["Report"])
